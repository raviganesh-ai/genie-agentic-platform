...
- **Professional Mission Input**: every generated form now sits on a deterministic light, high-contrast operational surface with a compact section hierarchy, flattened nested cards, standardized option rows, upload zones, controls, actions, visible focus, and mobile touch targets. Long agent names wrap in a vertical mobile execution pipeline instead of widening the page. Generated inline foreground/background colors fail materialization; all generation and Workshop-regeneration prompts require concise helper copy and semantic `role="alert"` plus `genie-error` validation messages.
