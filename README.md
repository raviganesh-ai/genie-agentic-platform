...
- **Run safety and diagnostics**: starting Deploy & Launch for a workflow that already has an active run returns that run instead of creating a duplicate. Automatic build repair now appears as active regeneration with an attempt counter, and an exhausted repair budget reports the actual deterministic validator evidence instead of a generic retry message. The UI no longer optimistically marks the pending backend phase as running while the real Foundry provisioning phase is still active.
