...
| `GENIE_FINOPS_COST_REPORT_ENABLED` | `false` | Enables the `finops-cost-report` step's real cost query (FinOps hub if configured, else Azure Cost Management); reports `available=false` when off |
| `GENIE_FINOPS_COST_REPORT_TIMEOUT_SECONDS` | `30` | Max seconds to wait for the Azure Cost Management REST API call |
| `GENIE_FINOPS_HUB_KUSTO_CLUSTER_URI` | *(none)* | Optional FinOps toolkit hub Azure Data Explorer (Kusto) cluster URI; when set alongside the database/query below, the cost step queries it first and falls back to Cost Management on any failure |
| `GENIE_FINOPS_HUB_KUSTO_DATABASE` | *(none)* | Kusto database name on the configured FinOps hub cluster |
| `GENIE_FINOPS_HUB_KUSTO_QUERY` | *(none)* | Operator-supplied KQL query run against the FinOps hub; must project `ResourceType`/`Cost` (optionally `Currency`) columns — Genie never assumes a specific FinOps toolkit/FOCUS schema |
| `GENIE_FINOPS_HUB_TIMEOUT_SECONDS` | `30` | Max seconds to wait for the FinOps hub Kusto REST API call |
