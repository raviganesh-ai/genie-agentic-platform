...
Exercises internal identity/session validation through the real ASGI app (confirms
...
async def test_start_allows_anonymous_internal_access(local_settings) -> None:
...
async def test_start_requires_a_valid_session(local_settings) -> None:
    app = create_app(settings=_with_deployment_config(local_settings))
...
async def test_list_deployments_requires_a_valid_session(local_settings) -> None:
    app = create_app(settings=_with_deployment_config(local_settings))

    async with app.router.lifespan_context(app):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            resp = await client.get("/sessions/does-not-exist/deploy-launch/")
            assert resp.status_code == 404


async def test_prototype_inventory_is_available_to_internal_user(local_settings) -> None:
    app = create_app(settings=_with_deployment_config(local_settings))

    async with app.router.lifespan_context(app):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
                allowed = await client.get("/api/admin/prototypes")
...
async def test_full_pipeline_runs_through_the_real_http_api(
    ...
    async with app.router.lifespan_context(app):
        ...
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            session_resp = await client.post("/sessions", json={"title": mission_title})
            ...
            for _ in range(80):
                poll_resp = await client.get(f"/sessions/{session_id}/deploy-launch/{run_id}")
            ...
            download_resp = await client.get(
                f"/sessions/{session_id}/deploy-launch/{run_id}/download"
            )
            ...
            delete_resp = await client.delete(f"/sessions/{session_id}/deploy-launch/{run_id}")
            assert delete_resp.status_code == 204
            missing_resp = await client.get(f"/sessions/{session_id}/deploy-launch/{run_id}")
