...
**Triage Mode** (sidebar toggle, off by default) renders a full-height, right-docked panel that polls `GET /sessions/{id}/governance/events` and filters to `agent_execution` events. Because `WorkflowStepExecutor.execute_step()` records that event the instant an individual agent call resolves — not once a whole (possibly multi-step) workflow run/resume call returns — the panel reflects the orchestrator's real agent-call order and timing, including cases where several ungated steps execute back-to-back within one HTTP call. Human-gated phases are marked as proceeded as soon as an approval action navigates into that phase, so the trace does not continue to request approval while the background resume call and its first SSE event are still catching up. Each feed item shows a gamified XP/level readout plus a truncated preview of that agent's real output (`detail.output_preview`); there is no synthetic or simulated activity.
...
Current workflows use the Foundry agents below. The registry also retains enabled
`security-assessment-agent` and `test-generation-agent` definitions for possible
future work, but no current workflow invokes them and they do not gate deployment.
...
| `finops-hub-agent` | FinOps Hub Agent | finops | `finops-hub-query-v1` | Optional Deploy & Launch cost-data source backed by the configured Azure MCP Kusto tool |

### The orchestrator delegation pattern

Every `solution-discovery-workflow` step invokes `genie-orchestrator`, never the specialist directly. `genie-orchestrator` calls the phase's registered Foundry function tool (`call_requirements_analyst`, `call_architecture_designer`, or `call_build_agent`, implemented in `app/agents/tools/orchestration_tools.py`) through the same `AzureAgentGateway` path as any other production agent call, then returns that specialist's output **verbatim** as the step result. Legacy security/test tools remain registered but no current workflow selects them. Tool-based delegation is used because the pinned `azure-ai-projects` SDK has no native Connected Agents mechanism for Prompt Agents.
...
- **`solution-discovery-workflow`** — three steps, all addressed to `genie-orchestrator` (see [delegation pattern](#the-orchestrator-delegation-pattern) above):
  ...
  The retired Requirement Fidelity, generated-test, and blocking security-gate steps are not part of this workflow or Deploy & Launch. Current launch safety comes from deterministic generated-code validation, Foundry provisioning, image builds, runtime readiness, and fail-closed Azure deployment checks.
...
| **4. Build** | Build Agent generates one traceable component at a time | Deterministic materialization, request-contract, responsive-UI, and Impeccable validation |
| **5. Workshop** | The user reviews artifacts and may regenerate a selected component | Human decision |
| **6. Deploy & Launch** | The deterministic pipeline provisions the isolated Azure prototype and verifies runtime readiness through APIM | Explicit Start action; every required Azure/runtime check fails closed |
...
`call_build_agent` (`app/agents/tools/orchestration_tools.py`) invokes the Build Agent **once per component** (`build-generation-component-v1`), in a fixed bottom-up order: each specialist agent module (alphabetical), then the Orchestrator module, then the UI component. The prompt hard-constrains every component to the architecture's own list: *"Every zone and every agent you generate code for MUST come directly from that exact list below — never invent, add, rename, or merge."* The UI component implements **only** the input zones named in step 2, using the deterministic control mapping and shell classes (`genie-form`, `genie-form-section`, `genie-form-grid`, `genie-field`, `genie-field-help`, `genie-actions`, `genie-dropzone`, and `genie-btn`). It receives the fixed `{ onSubmit: (message, attachments?) => void }` contract and never owns progress, output, or agent-status state because the deterministic shell does. Multi-control forms that omit the responsive section/grid structure are rejected and regenerated before Azure provisioning. On retry, `previous_build_output` preserves components that already succeeded. Workshop Center can regenerate one selected component without rebuilding everything else.
...
The generated-code boundary also fails closed before Azure provisioning. `code_materializer.materialize_build` requires a complete specialist + `class OrchestratorAgent:` + UI set, verifies that every generated specialist is referenced through `AGENT_FOUNDRY_NAMES` and has visible start/completion progress narration, and requires every generated UI (including button-only or custom-control surfaces) to call `onSubmit`. Direct browser calls to `/invoke` are rejected: the deterministic shell alone sends each submission to the deployed backend's `/invoke/stream` endpoint. Every file input must use `File.text()` and pass attachments through that handoff. Frontend integration also refuses to write or deploy runtime configuration without an absolute HTTP(S) backend URL. Browser code may validate presence, readability, non-empty content, and general file type only; schema and business rules belong to the generated backend/orchestrator after the complete attachment is persisted.

### 4. Human approval + full decision lineage at every hop
...
- **Retired identities and prototypes**: the application registration formerly used for interactive Genie authentication and the retired JeyDemo/DerekPoC Foundry agents have been deleted. The latest DerekPoC cleanup also removed its Azure resource groups and ACR repositories; post-cleanup inventory is zero across all three surfaces. The unused legacy `Genie-db-sp` application and principal remain because another directory user owns them. No active frontend, API, APIM policy, deployment script, or generated prototype depends on the retired authentication registration.
...
- **Runtime protocol**: generated backends now stream specialist hand-offs as dedicated `progress` SSE events. The generated mission shell keeps that narration separate from final business output, so a completed response no longer resets visibly completed agent nodes to pending.
- **Fail-closed generation**: materialization now rejects incomplete builds and disconnected UIs. Every build must contain specialists, an orchestrator, and a UI; every UI must call the shell's `onSubmit`; every specialist must be mapped through `AGENT_FOUNDRY_NAMES` inside a `MissionFoundryAgent` call, covered by a `FunctionTool` and awaited specialist run, and named in awaited start/completion `on_progress` calls. Frontend integration refuses missing, relative, or non-HTTP(S) backend URLs.
- **Cleanup**: all remaining DerekPoC Foundry agents were deleted; DerekPoC Azure resource-group and ACR repository inventories are also zero.
- **Verification**: focused generated-shell, materializer, pipeline, API, and prompt-contract tests pass; the full backend suite passes with 643 tests and Ruff is clean.

### 2026-09-25 — Generated prototype forms remain contained and readable

- **Incident**: a generated blind-evaluation prototype rendered native file controls and long checkbox labels beyond their CSS grid tracks. Upload zones overlapped neighboring model selectors, fieldsets collided with numeric inputs, and the optional glossary control extended outside the application surface.
- **Generator fix**: all Build Agent UI generation and repair prompts now require multi-control forms to use `genie-form-section`, `genie-form-grid`, and one containing `genie-field` per control. Materialization rejects and regenerates output that violates this responsive structure before any prototype reaches Azure. This is a Genie build-system invariant, not a patch to one generated application.
- **Shell fix**: the deterministic prototype shell constrains every input, select, textarea, file control, field, dropzone, fieldset, and legend to its assigned track. Checkbox/radio labels use wrapping flex rows, and the responsive grid favors three readable columns on desktop, two at intermediate widths, and one on mobile instead of forcing a cramped four-column form.
- **Verification**: generator, materializer, prompt-contract, and deployment-pipeline tests pass; Playwright rendered a representative upload/select/fieldset form at 1247px and 390px with zero overlapping elements, zero oversized field children, and no horizontal page overflow.
...
### 2026-09-10 — Gateway and authentication transition history (superseded)

Genie and generated prototypes moved through several same-day authenticated and direct-ingress designs before settling on anonymous browser access through dedicated APIM gateways with private FastAPI backends. The retired MISE sidecars, acceptance keys, direct public backend ingress, and corporate Entra browser flow described by those intermediate commits are not current behavior. Git history retains their implementation and incident detail; the current boundary is documented in [Authentication](#authentication) and [Deploy & Launch pipeline](#deploy--launch-pipeline).
...
### Retired Requirement Fidelity history

The generated acceptance-test fidelity gate and its name/JUnit matching fixes were removed on 2026-09-15 because the signal produced repeated false deployment failures. Git history retains the implementation details; the current deterministic launch checks are documented above.
