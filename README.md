...
- **Named-person Discovery**: **Find people** performs only one task: it lists the unique human names explicitly present in customer evidence. It does not infer roles, pain points, confidence, or other profile details before selection. Role archetypes, teams, organizations, products, and hypothetical users are excluded. Selecting **Investigate {name}** then starts a separate deep dive restricted to statements or authored material attributable to that person, covering their situation, pain points, risks, gaps, and clarification needs without assigning other participants' content to them. If no person is named, Genie asks for evidence that identifies one instead of inventing a persona.
