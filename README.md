...
| 7 | `execute-test-suite` | Requirement Fidelity Gate | Shared-auth deployments run from Genie through the prototype's APIM endpoint using a trusted loopback proxy that owns a per-prototype 256-bit acceptance key. The key is stored only as a Container App secret, an APIM secret named value, and transient trusted-process memory; generated code receives no credential. APIM removes caller-supplied internal authorization headers, admits the exact acceptance key or validates corporate Entra, removes the key before forwarding, and sends a separate proof header to private FastAPI for defense-in-depth validation. Launch requires the configured executable-coverage threshold and 100% passing evidence for all executable requirement tests. Failed, errored, skipped, timed-out, or unobserved executable tests still trigger automatic regeneration and redeployment up to `GENIE_DEPLOYMENT_FIDELITY_MAX_REPAIR_ATTEMPTS` times (default 3), then fail closed |
...
| `GENIE_PROTOTYPE_TEST_PRINCIPAL_CLIENT_ID` | *(none)* | Per-prototype authentication mode only: client id of Genie's user-assigned managed identity used to obtain short-lived application tokens. Shared corporate-tenant mode uses the isolated per-prototype acceptance key instead |
...
### 2026-09-10 — Preserve acceptance testing across APIM and tenant isolation
