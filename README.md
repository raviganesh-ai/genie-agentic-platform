...
| 6 | `security-copilot-scan` | Microsoft Security Copilot Scan | Invokes a pre-configured Microsoft Security Copilot Automated Action (Logic Apps HTTP trigger) against the mission's own deployed prototype and records its findings. **Informational-only**: honestly reports `available=false` when `GENIE_SECURITY_COPILOT_LOGIC_APP_URL` isn't configured or the call fails, and never fails or blocks the pipeline |
| 7 | `finops-cost-report` | Azure FinOps Cost Report | Queries the Azure Cost Management REST API directly (via `DefaultAzureCredential`, no extra SDK dependency) scoped to the mission's own resource group and records real spend. **Informational-only**: honestly reports `available=false` when `GENIE_FINOPS_COST_REPORT_ENABLED` is off or the call fails, and never fails or blocks the pipeline |
| 8 | `launch-mission` | Launch | Mints the customer-facing launch link once the prior deployment steps have completed |

Each step's real status (`pending` → `running` → `completed`/`failed`/`skipped`) streams live to the Deploy & Launch page so the human watches actual provisioning happen — never a simulated progress bar. Steps 6 and 7 (Security Copilot scan, FinOps cost report) are deliberately informational-only — unlike every other step, neither can ever fail the pipeline or block Launch; they surface real evidence for the human to review without gating the mission on it.
...
| `GENIE_SECURITY_COPILOT_LOGIC_APP_URL` | *(none)* | SAS-signed Logic Apps HTTP-trigger URL for the Microsoft Security Copilot Automated Action; the `security-copilot-scan` step reports `available=false` when unset |
| `GENIE_SECURITY_COPILOT_TIMEOUT_SECONDS` | `120` | Max seconds to wait for the Security Copilot Automated Action call |
| `GENIE_FINOPS_COST_REPORT_ENABLED` | `false` | Enables the `finops-cost-report` step's real Azure Cost Management query; reports `available=false` when off |
