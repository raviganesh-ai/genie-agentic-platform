...
- **Bounded recovery**: solution output is capped below 20,000 characters with limits on architecture text, requirements, nodes, edges, tradeoffs, references, and pricing queries. Live verification on revision `gh93` found a second boundary case where valid JSON still exceeded three 4,000-character narrative limits. The prompt now targets 1,200–2,500 characters per build narrative, and the single fresh Foundry retry receives the exact privacy-safe validation paths plus explicit correction limits. A second invalid response still restores `ready_for_solutions` and fails closed without local or static fallback.
