...
The generated-code boundary also fails closed before Azure provisioning. `code_materializer.materialize_build` requires the exact `class OrchestratorAgent:` contract, verifies that every generated specialist is referenced through `AGENT_FOUNDRY_NAMES` and has visible start/completion progress narration, rejects interactive UIs that never call `onSubmit`, rejects direct browser calls to `/invoke`, and requires every file input to use `File.text()` and pass attachments through the shell. Browser code may validate presence, readability, non-empty content, and general file type only; schema and business rules belong to the generated backend/orchestrator after the complete attachment is persisted.
...
- **Fail-closed generation**: materialization now parses the generated orchestrator and rejects a build unless every generated specialist is mapped through `AGENT_FOUNDRY_NAMES` inside a `MissionFoundryAgent` call, covered by a `FunctionTool` and awaited specialist run, and named in awaited start/completion `on_progress` calls.
...
- **Verification**: 91 focused generated-shell, materializer, pipeline, API, and prompt-contract tests pass; the full backend suite passes with 635 tests and Ruff is clean.
