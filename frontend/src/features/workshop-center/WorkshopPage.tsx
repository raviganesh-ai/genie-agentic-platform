...
export function WorkshopPage(): JSX.Element {
  ...
  // Live incremental content comes from the SSE workflow event stream
  ...
  useEffect(() => {
    ...
    setBuildStartStuck(false);
    // 5 minutes: generous enough that a legitimately slow first component
    // (large specialist agent, cold Foundry connection, etc.) never trips a
    // false "stuck" warning - this only ever fires when NOTHING has proven
