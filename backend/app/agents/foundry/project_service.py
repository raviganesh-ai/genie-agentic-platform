...
*constructing* that SDK client (using Microsoft Entra ID / managed identity
per the Security Requirements in ``.github/copilot-instructions.md``) and
handing back the ``AgentApiClient`` abstraction, never the raw SDK client.
"""
...
class FoundryProjectService:
    """Lazily constructs and caches the Azure AI Foundry project client.
